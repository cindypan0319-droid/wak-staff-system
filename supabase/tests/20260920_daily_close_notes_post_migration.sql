/* Migration 3 post-migration verification. READ ONLY. */

-- All four functions must exist with unchanged public RPC signatures.
SELECT
...
-- Public RPC privileges must remain unchanged; the helper must be private.
SELECT
  p.oid::regprocedure::text AS function_signature,
...
SELECT
  ...
  ) AS public_cannot_execute_notes_wrapper,
  NOT has_function_privilege(
    'anon',
    'public._wak_apply_daily_close_with_notes(jsonb,boolean,boolean)',
    'EXECUTE'
  ) AS anon_cannot_execute_notes_wrapper,
  NOT has_function_privilege(
    'authenticated',
    'public._wak_apply_daily_close_with_notes(jsonb,boolean,boolean)',
    'EXECUTE'
  ) AS authenticated_cannot_execute_notes_wrapper,
  NOT has_function_privilege(
    'service_role',
    'public._wak_apply_daily_close_with_notes(jsonb,boolean,boolean)',
    'EXECUTE'
  ) AS service_role_cannot_execute_notes_wrapper;

-- Compare public RPC ACL entries exactly with the pre-migration capture.
SELECT
  p.oid::regprocedure::text AS function_signature,
  CASE acl.grantee
    WHEN 0 THEN 'PUBLIC'
...
-- The table contract must remain unchanged.
SELECT
  c.ordinal_position,
  c.column_name,
  c.data_type,
...
-- Must match the pre-migration result exactly.
SELECT
