/* READ ONLY. Run after Migration 009. */
DO $verify$
DECLARE
  v_policy text;
...
BEGIN
  IF session_user <> 'postgres' THEN
    RAISE EXCEPTION 'M9_POST: run as postgres in Supabase SQL Editor';
  END IF;
  ...
  FOREACH v_policy IN ARRAY ARRAY[
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'M9_POST: removed policy % still exists', v_policy;
    END IF;
  END LOOP;
  ...
  FOREACH v_policy IN ARRAY ARRAY[
    ...
    'Owner/Manager can read all - time_clock',
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'M9_POST: preserved policy % is missing', v_policy;
    END IF;
  END LOOP;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'M9_POST: time_clock RLS is not enabled';
  END IF;
  ...
  IF v_clock_in IS NULL OR v_clock_out IS NULL
    ...
    RAISE EXCEPTION 'M9_POST: Phase A RPC/ACL contract changed';
  END IF;
  ...
  IF pg_catalog.has_table_privilege('authenticated', 'public.time_clock', 'SELECT') IS NOT TRUE
    ...
    RAISE EXCEPTION 'M9_POST: authenticated table privileges changed';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'M9_POST: time_clock_one_open changed or is missing';
  END IF;
END
$verify$;

/* Compare direct_acl with pre-migration; policy count is expected to be four lower. */
