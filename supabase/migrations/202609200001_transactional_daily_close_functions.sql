...
/*
 * Migration 1: additive transactional Daily Close infrastructure.
 *
 * This migration deliberately does not change existing tables, triggers,
 * views, data, grants on tables, or RLS policies. Existing application writes
 * therefore continue to work until they are migrated to these RPCs.
 */

CREATE OR REPLACE FUNCTION public._wak_canonical_platform(p_platform text)
RETURNS text
LANGUAGE sql
IMMUTABLE
STRICT
...
AS $function$
  SELECT CASE regexp_replace(upper(btrim(p_platform)), '[[:space:]]+', ' ', 'g')
    ...
    ELSE btrim(p_platform)
  END
$function$;

COMMENT ON FUNCTION public._wak_canonical_platform(text) IS
  'Internal canonical platform mapping matching set_platform_income_fees aliases.';

REVOKE ALL ON FUNCTION public._wak_canonical_platform(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._wak_canonical_platform(text) FROM anon;
REVOKE ALL ON FUNCTION public._wak_canonical_platform(text) FROM authenticated;


CREATE OR REPLACE FUNCTION public._wak_cash_counts_total(
  p_counts jsonb,
  p_context text
)
RETURNS numeric
...
DECLARE
  v_allowed_keys constant text[] := ARRAY[
    'note100', 'note50', 'note20', 'note10', 'note5',
    'coin2', 'coin1', 'coin50c', 'coin20c', 'coin10c', 'coin5c'
  ];
  v_key text;
  v_key_count integer;
  v_quantity numeric;
...
BEGIN
  ...
  SELECT count(*)
  INTO v_key_count
  FROM pg_catalog.jsonb_object_keys(p_counts);

  IF v_key_count <> cardinality(v_allowed_keys)
     OR EXISTS (
       SELECT 1
       FROM pg_catalog.jsonb_object_keys(p_counts) AS supplied(key)
       WHERE NOT (supplied.key = ANY (v_allowed_keys))
     ) THEN
    RAISE EXCEPTION
      '% must contain exactly the supported denomination keys', p_context
      USING ERRCODE = '22023';
  END IF;

  FOREACH v_key IN ARRAY v_allowed_keys LOOP
    IF NOT (p_counts ? v_key) THEN
      RAISE EXCEPTION '% is missing denomination key %', p_context, v_key
        USING ERRCODE = '22023';
    END IF;

    IF jsonb_typeof(p_counts -> v_key) <> 'number'
       OR (p_counts ->> v_key) !~ '^[0-9]+([.]0+)?$' THEN
      RAISE EXCEPTION
        '%.% must be a nonnegative whole-number JSON number', p_context, v_key
        USING ERRCODE = '22023';
    END IF;

    v_quantity := (p_counts ->> v_key)::numeric;
    v_multiplier := CASE v_key
      WHEN 'note100' THEN 100.00
      WHEN 'note50' THEN 50.00
      WHEN 'note20' THEN 20.00
      WHEN 'note10' THEN 10.00
      WHEN 'note5' THEN 5.00
      WHEN 'coin2' THEN 2.00
      WHEN 'coin1' THEN 1.00
      WHEN 'coin50c' THEN 0.50
      WHEN 'coin20c' THEN 0.20
      WHEN 'coin10c' THEN 0.10
      WHEN 'coin5c' THEN 0.05
    END;

    v_total := v_total + (v_quantity * v_multiplier);
  END LOOP;
...
COMMENT ON FUNCTION public._wak_cash_counts_total(jsonb, text) IS
  'Internal strict validator and total calculator for Daily Entry denominations.';

REVOKE ALL ON FUNCTION public._wak_cash_counts_total(jsonb, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._wak_cash_counts_total(jsonb, text) FROM anon;
REVOKE ALL ON FUNCTION public._wak_cash_counts_total(jsonb, text) FROM authenticated;


CREATE OR REPLACE FUNCTION public.save_morning_cashup(
...
DECLARE
  ...
  v_existing_entered_by uuid;
  v_committed public.cashup_sessions%ROWTYPE;
BEGIN
  v_actor := auth.uid();
  ...
  IF v_role NOT IN ('STAFF', 'MANAGER', 'OWNER') THEN
    RAISE EXCEPTION 'Role is not permitted to save morning cashup'
      USING ERRCODE = '42501';
  END IF;
  ...
  v_total_cash := public._wak_cash_counts_total(p_counts, 'morning_counts');

  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_store_id || ':' || p_business_date::text, 0)
  );
  ...
  IF v_role = 'STAFF'
    ...
    RAISE EXCEPTION
      'MORNING_AFTER_CLOSE_REQUIRES_MANAGER: STAFF cannot change MORNING after NIGHT exists'
      USING ERRCODE = '42501';
  END IF;

  SELECT c.entered_by
  INTO v_existing_entered_by
  FROM public.cashup_sessions AS c
  WHERE c.business_date = p_business_date
    AND c.store_id = p_store_id
  ...
  IF FOUND
     AND v_role = 'STAFF'
     AND v_existing_entered_by IS DISTINCT FROM v_actor THEN
    RAISE EXCEPTION
  ...
  RETURN jsonb_build_object(
    'store_id', v_committed.store_id,
    'business_date', v_committed.business_date,
    'session_type', v_committed.session_type,
    'counts', v_committed.counts,
    'total_cash', v_committed.total_cash,
    'removed_cash', v_committed.removed_cash,
    'entered_by', v_committed.entered_by,
    'created_at', v_committed.created_at,
    'updated_at', v_committed.updated_at
  );
END
$function$;

COMMENT ON FUNCTION public.save_morning_cashup(date, text, jsonb) IS
  'Atomically validates and upserts one MORNING cashup using auth.uid() as entered_by.';

REVOKE ALL ON FUNCTION public.save_morning_cashup(date, text, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.save_morning_cashup(date, text, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.save_morning_cashup(date, text, jsonb) TO authenticated;
...
BEGIN
  ...
  v_expected_cash := round(
    greatest(0::numeric, v_night_total - v_opening_float),
    2
  );
  v_total_sales := round(
  ...
  v_cash_variance := round(v_cash_sales - v_expected_cash, 2);
  v_removed_variance := round(v_removed_total - v_expected_cash, 2);
  ...
  RETURN jsonb_build_object(
    ...
    'entered_by', v_actor,
    'close_contract_version', 1
  );
END
$function$;

COMMENT ON FUNCTION public._wak_apply_daily_close(jsonb, boolean, boolean) IS
  'Internal transactional Daily Close implementation. Not executable by application roles.';

REVOKE ALL ON FUNCTION public._wak_apply_daily_close(jsonb, boolean, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._wak_apply_daily_close(jsonb, boolean, boolean) FROM anon;
REVOKE ALL ON FUNCTION public._wak_apply_daily_close(jsonb, boolean, boolean) FROM authenticated;
...
COMMENT ON FUNCTION public.submit_daily_close(jsonb) IS
  'Authenticated atomic Daily Close for active STAFF, MANAGER, or OWNER.';

REVOKE ALL ON FUNCTION public.submit_daily_close(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.submit_daily_close(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.submit_daily_close(jsonb) TO authenticated;
...
COMMENT ON FUNCTION public.correct_daily_close(jsonb) IS
  'Authenticated atomic Daily Close correction for active MANAGER or OWNER.';

REVOKE ALL ON FUNCTION public.correct_daily_close(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.correct_daily_close(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.correct_daily_close(jsonb) TO authenticated;
