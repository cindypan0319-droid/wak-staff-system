...
/* Migration 013 / P1B: coverage-aware interpretation for COVERED parent shifts. */
DO $precondition$
DECLARE
...
BEGIN
  IF session_user<>'postgres' THEN
    RAISE EXCEPTION 'MIGRATION_13_PRECONDITION: run as postgres';
  END IF;
  IF v_function IS NULL THEN
    RAISE EXCEPTION 'MIGRATION_13_PRECONDITION: Migration 011 generator is missing';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'MIGRATION_13_PRECONDITION: generator security contract differs';
  END IF;
  ...
  IF v_constraint IS NULL
    ...
    RAISE EXCEPTION 'MIGRATION_13_PRECONDITION: anomaly type constraint differs';
  END IF;
END
$precondition$;

ALTER TABLE public.work_period_anomalies
  DROP CONSTRAINT work_period_anomalies_type_check;
ALTER TABLE public.work_period_anomalies
  ADD CONSTRAINT work_period_anomalies_type_check CHECK (anomaly_type IN (
    ...
    'SOURCE_CHANGED_AFTER_REVIEW', 'COVERED_WITHOUT_COVER_SHIFT'
  ));
...
BEGIN
  ...
  IF v_function IS NULL OR v_constraint NOT ILIKE '%COVERED_WITHOUT_COVER_SHIFT%' THEN
    RAISE EXCEPTION 'MIGRATION_13_POSTCONDITION: function or anomaly contract missing';
  END IF;
  IF NOT EXISTS(
    SELECT 1 FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_roles r ON r.oid=p.proowner
    WHERE p.oid=v_function AND p.prosecdef AND r.rolname='postgres'
    ...
    RAISE EXCEPTION 'MIGRATION_13_POSTCONDITION: generator security differs';
  END IF;
  IF EXISTS(
    ...
    RAISE EXCEPTION 'MIGRATION_13_POSTCONDITION: generator ACL differs';
  END IF;
END
$postcondition$;
