...
/* Migration 015: allow the active Melbourne payroll week to refresh as-of one timestamp. */
DO $precondition$
DECLARE
  v_function oid:=to_regprocedure('public.wak_refresh_attendance_shadow(text,date,uuid)');
BEGIN
  IF session_user<>'postgres' THEN
    RAISE EXCEPTION 'MIGRATION_15_PRECONDITION: run as postgres';
  END IF;
  IF v_function IS NULL THEN
    RAISE EXCEPTION 'MIGRATION_15_PRECONDITION: Migration 013 generator is missing';
  END IF;
  IF NOT EXISTS(
  ...
  ) OR pg_catalog.pg_get_functiondef(v_function) NOT ILIKE '%ATTENDANCE_SHADOW_PERIOD_NOT_COMPLETE%'
     OR pg_catalog.pg_get_functiondef(v_function) ILIKE '%period_complete%' THEN
    RAISE EXCEPTION 'MIGRATION_15_PRECONDITION: generator contract differs from Migration 013';
  END IF;
END
$precondition$;

CREATE OR REPLACE FUNCTION public.wak_refresh_attendance_shadow(
  p_store_id text,
  p_week_start date,
  p_actor uuid
) RETURNS jsonb
...
DECLARE
  ...
  v_cover_child_count integer;
  v_candidate_missing_cover boolean;
  v_skip_fuzzy boolean;
...
BEGIN
  ...
  v_current_week_start:=(v_now AT TIME ZONE 'Australia/Melbourne')::date
    - ((extract(dow FROM (v_now AT TIME ZONE 'Australia/Melbourne')::date)::integer-4+7)%7);
  IF p_week_start>v_current_week_start THEN
    RAISE EXCEPTION 'ATTENDANCE_SHADOW_PERIOD_IN_FUTURE';
  END IF;
  v_period_complete:=p_week_start<v_current_week_start;
  v_scan_end:=CASE WHEN v_period_complete THEN v_period_end ELSE v_now END;

  SELECT upper(p.role::text) INTO v_actor_role
  FROM public.profiles p WHERE p.id=p_actor AND p.is_active IS TRUE;
  ...
  ON CONFLICT(store_id,week_start) DO UPDATE SET
    week_end=excluded.week_end,timezone=excluded.timezone,shadow_status='BUILDING',
  ...
  FOR v_clock IN
    ...
    WHERE tc.clock_in_at>=v_period_start AND tc.clock_in_at<v_scan_end
    ORDER BY tc.id FOR SHARE
  LOOP
    v_types:=ARRAY[]::text[];
    ...
    v_match_end:=NULL;
    v_candidate_count:=0;
    v_cover_child_count:=0;
    v_candidate_missing_cover:=false;
    v_skip_fuzzy:=false;
    v_actual_end:=CASE
      WHEN v_clock.clock_out_at IS NULL OR v_clock.clock_out_at>v_clock.clock_in_at
        THEN v_clock.clock_out_at ELSE NULL END;
    ...
    SELECT * INTO v_work_period FROM public.work_periods wp
    WHERE wp.time_clock_id=v_clock.id FOR UPDATE;
    IF FOUND THEN
      ...
      END IF;
      IF v_work_period.status='VOIDED' OR v_current.disposition='VOIDED'
    ...
    /* Explicit normal/child shifts retain M011 priority. COVERED parents use uncovered segments. */
    IF v_clock.shift_id IS NOT NULL THEN
      ...
      ELSIF FOUND AND v_explicit.store_id=p_store_id
        AND upper(coalesce(v_explicit.shift_status,'SCHEDULED')) IN ('SCHEDULED','WORKED') THEN
        v_match_id:=v_explicit.id;
        v_match_start:=v_explicit.shift_start;
        v_match_end:=v_explicit.shift_end;
      ELSIF FOUND AND v_explicit.store_id=p_store_id
        AND upper(coalesce(v_explicit.shift_status,'SCHEDULED'))='COVERED' THEN
        SELECT count(*) INTO v_cover_child_count
        FROM public.shifts child
        WHERE child.parent_shift_id=v_explicit.id AND child.store_id=p_store_id
          AND upper(coalesce(child.shift_status,'SCHEDULED')) IN ('SCHEDULED','WORKED')
          AND child.shift_end>child.shift_start
          AND child.shift_start<v_explicit.shift_end
          AND child.shift_end>v_explicit.shift_start;
        IF v_cover_child_count=0 THEN
          v_match_id:=v_explicit.id;
          v_match_start:=v_explicit.shift_start;
          v_match_end:=v_explicit.shift_end;
          v_types:=array_append(v_types,'COVERED_WITHOUT_COVER_SHIFT');
          v_severities:=array_append(v_severities,'BLOCKING');
          v_skip_fuzzy:=true;
        ELSE
          WITH child_coverage AS (
            ...
            SELECT lower(piece) AS segment_start,upper(piece) AS segment_end
            FROM child_coverage c
            CROSS JOIN LATERAL unnest(
              tstzmultirange(tstzrange(v_explicit.shift_start,v_explicit.shift_end,'[)'))
              - coalesce(c.covered,'{}'::tstzmultirange)
            ) AS uncovered(piece)
          )
          SELECT count(*),min(segment_start),min(segment_end)
          INTO v_candidate_count,v_match_start,v_match_end
          FROM segments
          WHERE abs(extract(epoch FROM (segment_start-v_clock.clock_in_at)))<=21600
            AND (v_clock.clock_out_at IS NULL
              OR v_clock.clock_out_at<=v_clock.clock_in_at
              OR (v_clock.clock_in_at<segment_end AND v_clock.clock_out_at>segment_start));
    ...
    /* Fuzzy discovery considers normal shifts plus each remaining COVERED-parent segment. */
    IF v_match_id IS NULL AND NOT v_skip_fuzzy
       AND NOT ('SHIFT_STAFF_MISMATCH'=ANY(v_types)) THEN
      WITH normal_segments AS (
        SELECT s.id AS shift_id,s.shift_start AS segment_start,s.shift_end AS segment_end,
          false AS coverage_aware,false AS missing_cover
        ...
        SELECT s.id,s.shift_start,s.shift_end
        FROM public.shifts s
        WHERE s.store_id=p_store_id AND s.staff_id=v_clock.staff_id
          AND upper(coalesce(s.shift_status,'SCHEDULED'))='COVERED'
        ...
        LEFT JOIN public.shifts child ON child.parent_shift_id=parent.id
          AND child.store_id=p_store_id
          AND upper(coalesce(child.shift_status,'SCHEDULED')) IN ('SCHEDULED','WORKED')
          AND child.shift_end>child.shift_start
          AND child.shift_start<parent.shift_end AND child.shift_end>parent.shift_start
        GROUP BY parent.id
      ), covered_segments AS (
        SELECT parent.id AS shift_id,lower(piece) AS segment_start,upper(piece) AS segment_end,
          true AS coverage_aware,false AS missing_cover
        FROM covered_parents parent JOIN coverage c ON c.parent_id=parent.id
        CROSS JOIN LATERAL unnest(
          tstzmultirange(tstzrange(parent.shift_start,parent.shift_end,'[)'))
          - coalesce(c.covered,'{}'::tstzmultirange)
        ) AS uncovered(piece)
        WHERE c.child_count>0
      ), missing_cover_segments AS (
        SELECT parent.id AS shift_id,parent.shift_start AS segment_start,
          parent.shift_end AS segment_end,true AS coverage_aware,true AS missing_cover
        FROM covered_parents parent JOIN coverage c ON c.parent_id=parent.id
        WHERE c.child_count=0
      ), candidates AS (
        SELECT * FROM normal_segments
        UNION ALL SELECT * FROM covered_segments
        UNION ALL SELECT * FROM missing_cover_segments
      )
      SELECT count(*),min(shift_id),min(segment_start),min(segment_end),bool_or(missing_cover)
      INTO v_candidate_count,v_match_id,v_match_start,v_match_end,v_candidate_missing_cover
      FROM candidates
      WHERE abs(extract(epoch FROM (segment_start-v_clock.clock_in_at)))<=21600
        AND (NOT coverage_aware OR v_clock.clock_out_at IS NULL
          OR v_clock.clock_out_at<=v_clock.clock_in_at
          OR (v_clock.clock_in_at<segment_end AND v_clock.clock_out_at>segment_start));
      IF v_candidate_count=1 THEN
        IF v_candidate_missing_cover THEN
          v_types:=array_append(v_types,'COVERED_WITHOUT_COVER_SHIFT');
          v_severities:=array_append(v_severities,'BLOCKING');
        END IF;
      ELSIF v_candidate_count>1 THEN
    ...
    IF EXISTS(
      ...
      WHERE other.id<>v_clock.id AND other.staff_id=v_clock.staff_id
        AND other.clock_in_at>=v_period_start AND other.clock_in_at<v_scan_end
    ...
    IF EXISTS(
      ...
      WHERE other.id<>v_clock.id AND other.staff_id=v_clock.staff_id
        AND other.clock_in_at>=v_period_start AND other.clock_in_at<v_scan_end
        AND other.clock_in_at<coalesce(v_clock.clock_out_at,v_scan_end)
        AND v_clock.clock_in_at<coalesce(other.clock_out_at,v_scan_end)
    ) THEN
      v_types:=array_append(v_types,'OVERLAPPING_CLOCK');
      v_severities:=array_append(v_severities,'BLOCKING');
    END IF;

    /* Existing +/-5 minute rules use the matched normal shift or uncovered segment boundaries. */
    IF v_match_id IS NOT NULL AND v_clock.clock_out_at IS NOT NULL
       AND v_clock.clock_out_at>v_clock.clock_in_at THEN
      IF v_clock.clock_in_at<v_match_start THEN
        IF extract(epoch FROM (v_match_start-v_clock.clock_in_at))<=300 THEN
          v_payable_start:=v_match_start;
        ELSE
      ...
      ELSE
        ...
        IF extract(epoch FROM (v_clock.clock_in_at-v_match_start))>300 THEN
          v_types:=array_append(v_types,'LATE_START');
          v_severities:=array_append(v_severities,'WARNING');
        END IF;
      END IF;

      IF v_clock.clock_out_at<v_match_end THEN
        v_payable_end:=v_clock.clock_out_at;
        IF extract(epoch FROM (v_match_end-v_clock.clock_out_at))>300 THEN
          v_types:=array_append(v_types,'EARLY_FINISH');
          v_severities:=array_append(v_severities,'WARNING');
        END IF;
      ELSIF v_clock.clock_out_at=v_match_end THEN
        v_payable_end:=v_clock.clock_out_at;
      ELSIF extract(epoch FROM (v_clock.clock_out_at-v_match_end))<=300 THEN
        v_payable_end:=v_match_end;
      ELSE
    ...
    IF v_types && ARRAY[
      ...
      'COVERED_WITHOUT_COVER_SHIFT'
    ]::text[] THEN
      v_payable_start:=NULL;
      v_payable_end:=NULL;
    END IF;

    SELECT EXISTS(SELECT 1 FROM unnest(v_severities) desired(severity)
      WHERE severity='BLOCKING') OR EXISTS(
      SELECT 1 FROM public.work_period_anomalies existing
      WHERE existing.work_period_id=v_work_period.id AND existing.status='OPEN'
    ...
    INTO v_desired_fingerprint FROM unnest(v_types,v_severities) desired(t,s);
    SELECT coalesce(string_agg(
      a.anomaly_type||':'||CASE
        WHEN a.details->>'detector' IS DISTINCT FROM 'ATTENDANCE_SHADOW_V1'
          AND a.anomaly_type=ANY(v_types)
        THEN v_severities[array_position(v_types,a.anomaly_type)] ELSE a.severity
      END,',' ORDER BY a.anomaly_type
    ),'') INTO v_open_fingerprint
    ...
    IF coalesce(array_length(v_types,1),0)>0 THEN
      FOR v_index IN 1..array_length(v_types,1) LOOP
        v_details:=jsonb_build_object(
          ...
          'matched_shift_id',v_match_id,'roster_start_at',v_match_start,
          'roster_end_at',v_match_end
        );
        IF v_types[v_index]='INVALID_CLOCK_RANGE' THEN
          v_details:=v_details||jsonb_build_object(
            'raw_clock_in_at',v_clock.clock_in_at,'raw_clock_out_at',v_clock.clock_out_at
          );
        END IF;
        IF NOT EXISTS(
  ...
  UPDATE public.payroll_periods SET
    shadow_status=CASE WHEN v_period_complete THEN 'READY_FOR_COMPARISON' ELSE 'BUILDING' END,
    last_refreshed_at=clock_timestamp()
  WHERE id=v_period_id;

  RETURN jsonb_build_object(
    'store_id',p_store_id,'week_start',p_week_start,'week_end',p_week_start+6,
    'period_complete',v_period_complete,'scan_end',v_scan_end,
...
DECLARE
  v_function oid:=to_regprocedure('public.wak_refresh_attendance_shadow(text,date,uuid)');
BEGIN
  IF v_function IS NULL OR pg_catalog.pg_get_functiondef(v_function) NOT ILIKE '%period_complete%'
     OR pg_catalog.pg_get_functiondef(v_function) NOT ILIKE '%ATTENDANCE_SHADOW_PERIOD_IN_FUTURE%' THEN
    RAISE EXCEPTION 'MIGRATION_15_POSTCONDITION: live generator contract missing';
  END IF;
  IF NOT EXISTS(
    SELECT 1 FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_roles r ON r.oid=p.proowner
    WHERE p.oid=v_function AND p.prosecdef AND r.rolname='postgres'
    ...
    RAISE EXCEPTION 'MIGRATION_15_POSTCONDITION: generator security differs';
  END IF;
  IF EXISTS(
    ...
    RAISE EXCEPTION 'MIGRATION_15_POSTCONDITION: generator ACL differs';
  END IF;
END
$postcondition$;
