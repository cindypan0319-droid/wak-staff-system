...
/* Restore the Migration 010 anomaly contract and the Migration 011 generator. */
DO $precondition$
BEGIN
  IF EXISTS(
    SELECT 1 FROM public.work_period_anomalies
    WHERE anomaly_type='COVERED_WITHOUT_COVER_SHIFT'
  ) THEN
    RAISE EXCEPTION 'MIGRATION_13_ROLLBACK_BLOCKED: coverage anomaly rows exist and cannot satisfy the prior constraint';
  END IF;
END
$precondition$;
